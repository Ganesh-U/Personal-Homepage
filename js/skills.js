...
function calculateAngle(index, total) {
  ...
  return offset + (index / Math.max(1, total <= 4 ? total - 1 : total)) * angleRange;
}
...
function updateSubNodePosition(node, containerRect) {
  const nodeRadius = 30;
  const targetX = Math.max(nodeRadius, Math.min(containerRect.width - nodeRadius, node.x));
  const targetY = Math.max(nodeRadius, Math.min(containerRect.height - nodeRadius, node.y));
...
function addInteractivity() {
  ...
  nodes.filter(n => !n.isMain).forEach(node => {
    node.element.addEventListener("click", (e) => e.stopPropagation());
  });
  
  container.addEventListener("click", () => {
    nodes.filter(n => n.isMain && n.expanded).forEach(node => {
      toggleNodeExpansion(node);
...
function toggleNodeExpansion(node) {
  ...
  nodes.filter(n => n.isMain && n.expanded && n !== node).forEach(otherNode => {
